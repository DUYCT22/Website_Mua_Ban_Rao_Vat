    ...
    public class ListingController : Controller
    {
        ...
        public ActionResult Index(int? categoryId, string sort, int page = 1, int pageSize = 12)
        {
            try
            {
                ...
                var totalCount = query.Count(); // Đếm tổng số bài
        ...
        public ActionResult LiByCate(string slug, string encryptedId)
        {
            ...
            try
            {
                ...
                if (Session["userId"] != null)
                {
                    ...
                    .Where(n => n.UserId != idUser && n.Category.ParentId == id && n.Status == 1 && n.User.Status == true)
                ...
                else
                {
                    if (HomeModel.Categories.Count != 0)
                    {
                        HomeModel.Listing = Entity.Listings
                        .Include("User").Include("Images")
                        .Where(n => n.Category.ParentId == id && n.Status == 1 && n.User.Status == true)
                        .OrderByDescending(n => n.CreatedAt)
                        .ToList();
                    }
                    else
                    {
                        HomeModel.Listing = Entity.Listings
                       .Include("User").Include("Images")
                       .Where(n => n.CategoryId == id && n.Status == 1 && n.User.Status == true)
                       .OrderByDescending(n => n.CreatedAt)
                       .ToList();
                    }
